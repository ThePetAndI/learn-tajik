 ...
 *
 * Карточку можно пролистать не читая. Поэтому сразу за ней идёт выбор буквы
 * среди похожих: ҳ рядом с х, қ рядом с к. Это упражнение, а не экзамен:
 * промах показывает верную букву, но попыткой не считается — ни жизнь,
 * ни звезда на букве, которую видят впервые, не теряются.
 */
...
export const alphabetIntroModule: ExerciseModule<AlphabetIntroExercise> = {
  ...
  mount(ex: AlphabetIntroExercise, ctx: ExerciseContext): ExerciseInstance {
    ...
    function choose(index: number): void {
      ...
      // без ctx.attempt: карточка буквы в счёт урока не идёт
      ctx.finish({
        correct,
