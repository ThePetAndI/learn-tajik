...
export interface HudHandle {
  ...
  pulse: (what: 'coins' | 'lives' | 'stars') => void;
}
...
export function createHud(opts: HudOpts = {}): HudHandle {
  ...
  const el = h('header', { class: 'hud' }, livesPill, coinsPill, starsPill);
  ...
  function pulse(what: 'coins' | 'lives' | 'stars'): void {
    const target = what === 'coins' ? coinsPill : what === 'lives' ? livesPill : starsPill;
